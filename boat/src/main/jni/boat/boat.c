...
ANativeWindow *boatGetNativeWindow() {
    //__android_log_print(ANDROID_LOG_ERROR, TAG, "boatGetNativeWindow : %p", mBoat.window);
    return mBoat.window;
}
...
void boatGetCurrentEvent(BoatInputEvent *event) {
    ...
    //__android_log_print(ANDROID_LOG_ERROR, TAG, "", mBoat.window);
}
...
void boatSetCursorMode(int mode) {
    ...
    if (result != JNI_OK || env == 0) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "Failed to attach thread to JavaVM.");
        abort();
    }
    ...
    if (class_BoatInput == 0) {
        __android_log_print(ANDROID_LOG_ERROR, TAG,
                            "Failed to find class: cosine/boat/BoatInput.");
        abort();
    }

    jmethodID BoatInput_setCursorMode = (*env)->GetStaticMethodID(env, class_BoatInput,
                                                                  "setCursorMode", "(I)V");

    if (BoatInput_setCursorMode == 0) {
        __android_log_print(ANDROID_LOG_ERROR, TAG,
                            "Failed to find static method BoatInput::setCursorMode");
        abort();
    }
...
void sendKeyEvent(AInputEvent *aie) {
    ...
    jmethodID id = (*env)->GetStaticMethodID(env, class_BoatInput, "dispatchKeyEvent",
                                             "(JJIIIIIIII)V");
    (*env)->CallStaticVoidMethod(env, class_BoatInput, id,
                                 AKeyEvent_getDownTime(aie), AKeyEvent_getEventTime(aie),
                                 AKeyEvent_getAction(aie), AKeyEvent_getKeyCode(aie),
                                 AKeyEvent_getRepeatCount(aie),
                                 AKeyEvent_getMetaState(aie), AInputEvent_getDeviceId(aie),
                                 AKeyEvent_getScanCode(aie),
